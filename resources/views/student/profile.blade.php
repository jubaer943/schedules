...
@section('content')
    <div class="max-w-2xl bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
        ...
        <form action="{{ route('profile.update') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="name" value="{{ auth()->user()->name }}"
                        class="w-full mt-2 p-3 rounded-xl bg-gray-50 border-none">
                    @error('name')
                ...
                <div>
                    ...
                    <input type="email" disabled value="{{ auth()->user()->email }}"
                        class="w-full mt-2 p-3 rounded-xl bg-gray-200 border-none cursor-not-allowed">
                </div>
            </div>
            ...
            <div class="pt-6 border-t border-gray-50">
                ...
                <div class="space-y-4">
                    <input type="password" name="password" placeholder="New Password"
                        class="w-full p-4 rounded-xl border border-gray-100 focus:ring-2 focus:ring-blue-500 outline-none">
                    @error('password')
                        <span class="text-red-500 text-xs">{{ $message }}</span>
                    @enderror
            ...
            <button type="submit"
                class="bg-blue-800 text-white px-8 py-4 rounded-2xl font-bold shadow-lg shadow-blue-100 hover:bg-black transition">
                Save Changes
            </button>
        </form>
    </div>
