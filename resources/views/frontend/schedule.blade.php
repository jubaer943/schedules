...
<body class="bg-gray-100 font-sans">
    ...
    <div class="max-w-5xl mx-auto py-10 px-4">
        <form action="{{ route('schedule.appointment') }}" method="POST">
            @csrf
            <div
                class="bg-blue-800 text-white p-6 rounded-t-3xl shadow-xl flex flex-col md:flex-row justify-between items-center gap-4">
                <div class="flex items-center gap-4">
                    <div class="flex items-center gap-2 bg-white p-2 rounded-lg">
                        <img src="{{ asset('image/oasis-logo.png') }}" alt="Logo"
                ...
                <div class="flex items-center gap-6 text-sm">
                    ...
                    <div class="flex flex-col items-center space-y-1">
                        <span class="text-blue-400 uppercase text-[10px] font-semibold tracking-wider">Select
                            Date</span>
                        <div class="relative">
                            <input type="text" id="date-picker" placeholder="Choose Date" readonly
                                class="bg-blue-900/50 border-b-2 border-yellow-400 text-white font-bold py-1 px-3 focus:outline-none cursor-pointer rounded-t-md w-32 text-center">
                            <i
                                class="fas fa-calendar-day absolute right-2 top-2 text-yellow-400 pointer-events-none text-xs"></i>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white p-6 shadow-2xl border-x border-b border-gray-200">
                <h2 class="text-lg font-bold text-gray-700 mb-6 flex items-center gap-2">
                    ...
                    @auth
                        <a href="{{ route('dashboard') }}"
                            class="ml-auto text-sm font-medium text-blue-600 hover:text-blue-800 transition-colors">
                            Go to My Bookings
                        </a>
                    @endauth
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4" id="result-display">
                    <div
                        class="col-span-full text-center py-12 px-4 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200">
                ...
                @if (session('error'))
                    <div x-data="{ show: true }" x-show="show"
                        class="flex items-start p-4 mb-4 bg-red-50 border-l-4 border-red-500 rounded-r-lg shadow-sm">
            ...
            <div
                class="bg-gray-50 p-6 rounded-b-3xl border-x border-b border-gray-200 flex flex-col md:flex-row justify-between items-center gap-6">
                <div class="flex items-center gap-4">
                    <!-- <div class="text-3xl font-mono text-gray-300 border-r pr-4 italic">Ticket</div> -->
                    <div class="max-w-md">
                        <label for="email"
                            class="block text-xs text-gray-400 uppercase font-bold tracking-widest mb-2 ml-1">
                            Register Schedule
                        </label>
                        ...
                        <div class="relative flex items-center">
                            ...
                            <input type="email" name="email"
                                value="{{ Auth::check() ? Auth::user()->email : null }}" id="email"
                            ...
                            <button type="submit"
                                class="absolute right-2 bg-gray-900 hover:bg-black text-white px-4 py-1.5 rounded-lg text-sm font-semibold transition-colors">
                                Confirm
                            </button>
                        </div>
        ...
        <div class="mt-8 flex justify-center gap-8 text-[10px] font-bold text-gray-400 uppercase tracking-widest">
            <div class="flex items-center gap-2"><span class="w-4 h-4 bg-white border border-gray-200 rounded"></span>
                Available</div>
        ...
        const availableDates = [
            @if ($schedules)
                @foreach ($schedules as $schedule)
                    // Use ->format() because $schedule->date is now a Carbon object
        ...
        const dateMap = {
            @if ($schedules)
                @foreach ($schedules as $schedule)
                    ...
                    "{{ $schedule->date->format('Y-m-d') }}": "{{ $schedule->id }}",
                @endforeach
            @endif
        };
